...
void cb(uvc_frame_t *frame, void *ptr) {
  ...
  unsigned char* dataBuffer = (unsigned char*) bgr->data;
  Mat image(Size(bgr->width, bgr->height), CV_8UC3, dataBuffer, Mat::AUTO_STEP);
  imshow("image", image);
    ...
    if (res < 0) {
      ...
      if (res < 0) {
        ...
        if (res < 0) {
          ...
          sleep(100000000); /* stream for 100000000 seconds */
