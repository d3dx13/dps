...
def pub_array(data, affinity):
    ...
    while True:
        ...
        print(f"pub {affinity} freq = {np.prod(tensor.shape) * tensor.dtype.itemsize / (t_end - t_start):.6f} Gb pub / sec")
...
def sub_array(data, affinity):
    ...
    while True:
        ...
        data_tensor[:] = data_array[:]
        t_end = time.monotonic_ns()
        print(f"sub {affinity} freq = {np.prod(tensor.shape) * tensor.dtype.itemsize / (t_end - t_start):.6f} Gb pub / sec")
...
if __name__ == '__main__':
    ...
    pub_1_p = multiprocessing.Process(target=pub_array, args=(pub_1_array, [4, ]))
    pub_1_p.start()

    sub_1_p = multiprocessing.Process(target=sub_array, args=(pub_1_array, [5, ]))
    sub_1_p.start()
